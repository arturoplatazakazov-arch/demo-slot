"""Seed data for the "country-gold-3" game (front/country-gold-3.html).

Third slot in the project's 3x3 classic family (after lucky-miami-3 and
miami-fruits-3) and the same deliberately bare shape: eight symbols, five
paylines, LINE PAY AND NOTHING ELSE. No wild, no scatter, no free spins, no
bonus buy — the config carries zero FeatureConfig rows, which
app/api/v1/spin_service.py handles natively (every feature there is guarded by
`find_feature_config(...) is not None`).

New here is only the dress: a country/ranch theme drawn to a delivered art
reference, with the classic fruit-machine symbol set the product asked for
(seven, star, BAR, bell, grape, lemon, orange, plum).
...
_PAYS below reuses the weight/payout ladder solved against that identity in
app/seed/lucky_miami_3.py — same multiset of (weight, pay) pairs, just mapped
onto this game's symbols — so the RTP is the same 0.9557 by construction.
scripts/simulate.py is the check, not the search, and a single run will not
agree closely: the seven lands ~1 spin in 11 600 and carries 9% of the RTP, so
even a 1M-spin run has a standard error of ~0.012.

Symbol codes are a fixed contract with SYMBOL_CODES in
front/js/country-gold-3/slot.js (each code == its PNG file name in
front/img/country-gold-3/symbols/) — do not rename them.
"""
...
GAME_CODE = "country-gold-3"
GAME_NAME = "Country Gold 3"
CATALOG_BADGE = "Классика 3x3"
CATALOG_DESCRIPTION = "Золотое ранчо на закате: семёрки, BAR и пять линий — классика без бонусов."
CATALOG_COVER_PATH = "img/country-gold-3/img/cover.jpg"
CATALOG_PLAY_URL = "country-gold-3.html"
...
# is _PAYS / 5 (plum 9 = 1.8x the bet, seven 5000 = 1000x, the top win).
#
# Only a full 3-of-a-kind pays, which lands on ~16% of spins, so the average
# winning spin has to be worth ~6x the bet for the game to return anything at
# all. The weights are deliberately skewed (plum 10x commoner than seven) rather
# than flat — a flat reel would drop the hit rate to ~7.5% and make the game feel
# dead between wins.
_PAYS = {
    "plum": {"3": 9.0},
    "orange": {"3": 14.0},
...
_SYMBOLS: list[tuple[str, str, str, int]] = [
    ("plum", "Слива", "low", 40),
    ("orange", "Апельсин", "low", 32),
    ("lemon", "Лимон", "low", 26),
    ("grape", "Виноград", "low", 21),
    ("bell", "Колокольчик", "high", 15),
    ("star", "Звезда", "high", 10),
    ("bar", "BAR", "high", 7),
    ("seven", "Семёрка", "high", 4),
]
