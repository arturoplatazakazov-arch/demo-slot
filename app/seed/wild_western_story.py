"""Seed data for the "wild-western-story" demo game (front/wild-western-story.html
under this repo).

Symbol codes and the row-major grid orientation are a fixed contract with the
already-built frontend (front/js/wild-western-story/slot.js's SYMBOL_FOLDERS) —
...
Mechanics (per front/js/wild-western-story/slot.js's own header comment: this
game is East Discovery's frontend adapted MINUS coins/Hold & Win) — line-pay +
scatter free spins + expanding wild only:
...
GAME_CODE = "wild-western-story"
CATALOG_BADGE = "Free Spins"
CATALOG_DESCRIPTION = "Слот о Диком Западе с расширяющимся Wild и фриспинами"
CATALOG_COVER_PATH = "img/wild-western-story/img/logo_wildwestern-hero.jpg"
...
def build_game_config() -> tuple[Game, GameConfig]:
    ...
    game = Game(
        code=GAME_CODE, name="Wild Western Story",
    ...
    for order, (code, symbol_type, tier, weight, pays, max_per_reel) in enumerate(_SYMBOLS):
        Symbol(
            game_config=config,
            code=code,
            name=code.upper() if code in ("a", "k", "q", "j") else code.replace("_", " ").title(),
