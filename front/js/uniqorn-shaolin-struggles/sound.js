// Uniqorn Shaolin Struggles — central sound manager, те же механики, что у
// остальных игр (перекрывающиеся one-shot SFX через WebAudio + зацикленная
// музыка, разблокируемая первым жестом; отсутствующий файл падает мягко).
//
// Собственного звукового пака у темы пока нет, поэтому играем чужие (тот же
// приём, что в uniqorn-back-to-fabulous/sound.js): тиры выигрыша и музыка — из
// neon-reels, монета и «итог раунда» — из east-discovery (единственная другая
// игра с Hold & Win, у неё эти два звука как раз есть), UI и барабаны — общие.
...
const SOUND_FILES = {
  ...
  coinLand: EAST + 'HOLD&WIN_Coin.wav',
  roundTotalWin: EAST + 'Bonus_spins_total_win.mp3',
  // Награда джекпота — своего файла ни у кого нет, играем «эпик» (404 здесь
  // был бы тишиной ровно на самом громком моменте игры).
  jackpotWin: NEON + 'WIN_Epic.mp3',
