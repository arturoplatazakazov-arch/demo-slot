// Country Gold 3 — 3x3 classic, five lines, no bonus round.
//
// Сцена и движение барабанов взяты у ../lucky-miami-3 (та же 3x3 раскладка, тот
// же дизайн-канвас, та же подача целиком на PNG + CSS): в игре нет ни монет с
// номиналом, ни Hold & Win, ни фриспинов, ни покупки, поэтому нет ни второго
...
//
// Отличия от донора:
//   * КОРПУС НАРИСОВАН (img/cabinet.png) и лежит поверх барабанов; окно
//     обрезки одно на все три ленты и выставлено в его проём из CSS, поэтому
//     здесь нет ни трёх отдельных окон, ни огоньков между ними;
...
const ASSET_ROOT = 'img/country-gold-3';
...
// замене файла в img/country-gold-3/.
...
// app/seed/country_gold_3.py: код == имя файла в symbols/.
const SYMBOL_CODES = [
  'plum', 'orange', 'lemon', 'grape', 'bell', 'star', 'bar', 'seven',
];
...
// Зеркалит PAYLINES в app/seed/country_gold_3.py — нужен только для антиципации
...
function applyStaticContentOffset(img, code) {
  ...
  const override = window.SlotCalibration && window.SlotCalibration.get('country-gold-3', code);
...
function createCellNode(code) {
  ...
  img.addEventListener('error', () => {
    ...
    console.warn(`[country] no art for symbol code "${code}"`);
  }, { once: true });
...
function playPopup(key, amount = 0, holdMs = 2500) {
  ...
  return new Promise((resolve) => {
    ...
    (async () => {
      ...
      console.error(`[country] popup "${key}" failed — closing it so the screen isn't left dimmed:`, err);
      finish();
    });
  });
}
