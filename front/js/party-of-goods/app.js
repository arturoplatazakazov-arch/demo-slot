// Party Gods — bottom UI bar + spin orchestration, adapted from
// ../east-discovery/app.js (same GAME_ID/session/Api conventions), trimmed
...
// FeatureConfig rows, see app/seed/party_of_goods.py).

const GAME_ID = 'party-of-goods';
...
async function applySpinResult(data, totalWinFromCascades) {
  ...
  if (data.popup) {
    ...
    // Discovery's base-game wins). No sound here: the escalating WIN_Small
    // already fired per winning combination during the cascade (see slot.js).
...
async function runSpin(request) {
  ...
  // SPIN_Start fires 0.1s after this press; the Element_drop patter fires
  // later, from spinStartTransition when the grid lands (same as Sugar Galaxy).
  startSpinPressCue();
...
function renderInfoPopupContent() {
  ...
  for (const symbol of gameState.symbols) {
    ...
    row.innerHTML = `
      ${folder ? `<img src="img/party-of-goods/Export/${folder}/static.png" alt="${symbol.name}">` : ''}
