...
function startReelLoop() {
  ...
  for (let col = 0; col < GRID_COLS; col++) {
    ...
    // Read what's actually on screen for this column before wiping the
    // strip, so the loop's first cycle scrolls away from the current
    // symbols instead of popping straight to random filler.
    ...
    void stripEl.offsetHeight; // force reflow so the reset isn't merged with the loop start
