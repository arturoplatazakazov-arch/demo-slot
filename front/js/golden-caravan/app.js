// Golden Caravan — bottom UI bar + spin orchestration, adapted from
// ../sugar-galaxy/app.js (same GAME_ID/session/Api conventions, same avalanche
// cascade play loop). Features: avalanche cascades (slot.js:playAvalanche),
// free_spins + bonus_buy (see app/seed/golden_caravan.py).

const GAME_ID = 'golden-caravan';
...
async function applySpinResult(data, totalWinFromCascades) {
  ...
  // Every winning combination's own cascade animation has already finished
  // (playAvalanche ran before applySpinResult); now the scatters' own win
  // celebration, and only THEN the transition into bonus mode — never
  // simultaneously. Covers both a trigger from the initial grid and one where
  // the 3rd+ scatter only showed up via a cascade's own refill.
  ...
  // bonus still shows its own bonusSpinsWin popup normally.
  ...
  if (data.popup) {
    ...
    // the amount inline over the grid instead. No sound here: WIN_Small
    // already fired per winning combination during the cascade.
...
async function runSpin(request) {
  ...
  // SPIN_Start fires 0.1s after this press — independent of network latency.
  startSpinPressCue();
...
// ascending. Multiplier tokens (x2/x3/x5) and scatter carry an empty paytable
// and are skipped here — their role is explained in the note above the list.
function renderInfoPopupContent() {
  const list = document.getElementById('infoPopupList');
  if (!list) return;
  list.innerHTML = '';
  for (const symbol of gameState.symbols) {
    const folder = STATIC_FOLDERS[symbol.code];
    // Skip symbols that aren't part of this game's real art set — the builder's
    // aborted pass left zero-weight placeholder rows (low_a/high_a/boom) in the
    // config; they never draw, and have no STATIC_FOLDERS entry.
    if (!folder) continue;
    ...
    row.innerHTML = `
      <img src="img/golden-caravan/Export/${folder}/static.png" alt="${symbol.name}">
