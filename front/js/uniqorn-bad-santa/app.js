// Uniqorn Bad Santa — bottom UI bar + spin orchestration, adapted from
// ../sugar-galaxy/app.js (same session/Api conventions and the same feature
// set: avalanche cascades instead of a spinning reel, free_spins + bonus_buy
// only — see app/seed/uniqorn_bad_santa.py).

const GAME_ID = 'uniqorn-bad-santa';
...
// Shared tail end of both a normal spin and a bonus-buy response, called *after*
// slot.js:playAvalanche has already animated every cascade step — updates
// balance/free-spins mode, then shows whatever popup/inline amount the server
// sent.
async function applySpinResult(data, totalWinFromCascades) {
  ...
  // Order matters (product decision carried over from Sugar Galaxy): every
  // winning combination's cascade animation plays out first (already finished —
  // playAvalanche ran before this), THEN the scatters' own celebration, and only
  // THEN the transition into bonus mode — never simultaneously. Covers both a
  // trigger from the initial grid and one where the 3rd scatter only showed up
  // via a cascade refill.
  if (data.feature && data.feature.triggered) {
    await playScatterTriggerCelebration();
  }

  // Entering the bonus from the base game: setFreeSpinsMode runs the full intro
  // (blackout + bonusSpinsWin popup + reveal), so the popup block below must not
  // play that same popup a second time. A re-trigger *inside* the bonus still
  // shows its own popup normally (the mode is already 'freespins' by then).
  ...
  if (remaining > 0) {
    ...
    // Session-wide accumulator — NOT the same number as the base game's
    // per-step multiplier this spin's own cascade already showed.
  ...
  if (data.popup) {
    ...
    // Wins below the big/mega/epic threshold never get a popup — show the amount
    // inline over the grid instead. No sound here: WIN_Small already fired per
...
async function runSpin(request) {
  ...
  // SPIN_Start fires 0.1s after this press, independent of network latency; the
  // Element_drop patter fires later, when the grid actually lands.
  startSpinPressCue();
...
async function bootstrapSession() {
  try {
    ...
    // No backend reachable — leave the static placeholder values on screen (the
...
// rather than exact 3/4/5 counts, so every key in `paytable` is shown, sorted
// ascending. Multiplier baubles, the bomb and the scatter carry an empty
// paytable and drop out here — their role is explained in the note above the
// list (see the .info-popup__note markup in the page).
