// Wild Western Story — bottom UI bar + spin orchestration, adapted from
// ../east-discovery/app.js MINUS the Hold & Win / coin flow. Session
// bootstraps on load; every Spin/Buy click is a real request. No backend game
// exists for this GAME_ID yet, so bootstrapSession's catch just leaves the
// static placeholder values on screen (asset/animation preview still works).

const GAME_ID = 'wild-western-story';
...
async function bootstrapSession() {
  try {
    ...
    // No wild-western-story game exists on the backend yet — leave the static
    // placeholder values on screen rather than hard-failing.
...
// session-start); empty until a real wild-western-story backend game exists.
function renderInfoPopupContent() {
  ...
  for (const symbol of gameState.symbols) {
    ...
    const payParts = ['3', '4', '5']
      ...
      .join('');
    row.innerHTML = `
      ${folder ? `<img src="img/wild-western-story/Export/${folder}/${staticFileFor(symbol.code)}" alt="${symbol.name}">` : ''}
