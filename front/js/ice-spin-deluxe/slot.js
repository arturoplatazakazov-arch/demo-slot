// Ice Spin Deluxe — 3x3 classic, five lines, no bonus round.
//
// Сцена и движение барабанов взяты у ../country-gold-3 (та же 3x3 раскладка,
// тот же дизайн-канвас, та же подача целиком на PNG + CSS): в игре нет ни монет
// с номиналом, ни Hold & Win, ни фриспинов, ни покупки, поэтому нет ни второго
// режима экрана, ни поклеточной прокрутки. Остаётся ровно спин: увезти старые
// символы вниз, дождаться сервера, уронить новые, подсветить выигравшие линии.
//
// Отличия от донора:
//   * СИМВОЛОВ ДЕВЯТЬ, а не восемь — столько пришло листом от художника
//     (вишня добавлена в низ фруктовой лестницы);
//   * в раме ТРИ отдельных проёма с перемычками, поэтому обрезает не общий
//     контейнер, а каждая .reel__col по себе (см. css/ice-spin-deluxe.css);
//   * высота ячейки не читается из --cell-h (это calc от долей проёма, а
//     getComputedStyle отдаёт такие значения невычисленными), а МЕРЯЕТСЯ по
//     фактическому окну — единственный источник правды остаётся в CSS;
//   * кадр после сброса лент ждёт afterFrame(), а не голый двойной rAF: в
//     скрытой вкладке кадров нет вовсе, и спин, начатый перед уходом со
//     вкладки, повисал бы навсегда.

const ASSET_ROOT = 'img/ice-spin-deluxe';

// Версия арта в URL. Статика раздаётся без Cache-Control, поэтому браузер
// держит символы и раму по эвристике и после перерисовки показывает старую
// картинку. Поднимать при ЛЮБОЙ замене файла в img/ice-spin-deluxe/.
...
// app/seed/ice_spin_deluxe.py: код == имя файла в symbols/.
const SYMBOL_CODES = [
...
// Зеркалит PAYLINES в app/seed/ice_spin_deluxe.py — нужен только для антиципации
...
// Раскладка с референса игры: виноград столбом, BAR в центре нижнего ряда.
const SYMBOL_LAYOUT = [
  ['grape', 'cherry', 'plum'],
  ['grape', 'lemon', 'orange'],
  ['grape', 'bar', 'cherry'],
...
function applyStaticContentOffset(img, code) {
  ...
  const override = window.SlotCalibration && window.SlotCalibration.get('ice-spin-deluxe', code);
...
function createCellNode(code) {
  ...
  img.addEventListener('error', () => {
    ...
    console.warn(`[ice] no art for symbol code "${code}"`);
  }, { once: true });
...
// Блок проёмов (#reel) — три колонки, каждая режет свою ленту сама.
...
// the ones the SVG can literally draw a straight-segment line over. Overlays
// #reel (the shared flex container for all three .reel__col — see the
// module docstring on why each column clips independently but this parent
// still spans all three), so it draws straight through the frame's
// perempichki between openings same as it would over an unbroken window.
...
// promise from re-arming the timeout afterwards without this (see
// uniqorn-scandal/slot.js for the incident this guards against).
let winLineToken = 0;
...
// Cell centres in #reel's own coordinate space, measured live (not computed
// from row/col arithmetic) since the stage is CSS-scaled and the three
// window openings aren't a plain even split.
...
// symbols use, so the line pulses in lockstep with their glow.
...
function playMultiLineWinSequence(groups, allWinInfos, lineWins) {
  ...
  const step = () => {
    ...
    // groups[i] соответствует lineWins[i] 1:1 (line-выигрыши идут перед
    // count-выигрышами в ReelMath.collectWinGroups) — фаза -1 мешает линии
    // друг с другом, поэтому там своей линии не рисуем.
...
function playWinCells(winningCells, lineWins, countWins) {
  ...
  if (groups.length > 1) {
    ...
    // Один групп: это line-выигрыш, только если lineWins ровно из одной
    // записи (у одиночного count-pay/scatter выигрыша формы линии нет).
...
// спинов, и антиципация на каждой второй сливе перестала бы что-либо значить.
const ANTICIPATION_CODES = new Set(['bell', 'star', 'bar', 'seven']);
...
function playPopup(key, amount = 0, holdMs = 2500) {
  ...
  return new Promise((resolve) => {
    ...
    (async () => {
      ...
      console.error(`[ice] popup "${key}" failed — closing it so the screen isn't left dimmed:`, err);
      finish();
    });
  });
}
...
const DESIGN = { desktop: { w: 1600, h: 1000 }, mobile: { w: 780, h: 1080 } };
...
// Под устройство меняется только ФОН: рама на обеих раскладках одна и та же
// (cabinet.png) — своя портретная картинка со своими проёмами делала бы из
// слота другую игру, там рама и ячейка были бы другой пропорции.
...
async function init() {
  ...
  await preloadAssets();
  // Рама грузится позже сетки, а высота ячейки меряется по окну — после
