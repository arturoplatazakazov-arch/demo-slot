// Big Catch — bottom UI bar + spin orchestration, adapted from
// ../western-treasure/app.js минус его колесо фортуны: у этой игры механика
// Amy's Fruit Farm. Session bootstraps on load; every Spin/Buy click is a real
// request against app/seed/big_catch.py's config.
const GAME_ID = 'big-catch';
...
// cost_multiplier продукта bonus_buy в app/seed/big_catch.py. Число нужно
// только чтобы нарисовать цену на указателе и в диалоге; списывает сервер по
// своей конфигурации, что бы тут ни было показано.
const BUY_BONUS_COST_MULTIPLIER = 100;
// spins_awarded_by_count["3"] оттуда же — столько спинов даёт покупка.
const BUY_BONUS_SPINS = 10;
...
// Цена на нижней доске указателя. В макете там стоял статичный плейсхолдер
// «10 000 000 USD», в игре она равна ставке x100 и едет вместе со ставкой.
...
async function applySpinResult(data) {
  ...
  if (data.popup) {
    // Дважды не показываем: bonusSpinsWin на входе в бонус рисует сама
    // intro-перебивка (ретриггер внутри бонуса — как обычно, здесь), а
    // buyFreeSpins игрок уже видел диалогом, через который и купил.
    const shownByIntro = (enteringBonus && data.popup.type === 'bonusSpinsWin')
      || data.popup.type === 'buyFreeSpins';
    if (!shownByIntro) {
...
async function runSpin(request) {
  ...
  try {
    ...
    const anticipationColumns = gameState.freeSpinsTrigger
      ...
      : [];
    await landReels(data.grid, anticipationColumns, data.line_wins || [], data.count_wins || []);
...
function setupBuyBonusButton() {
  ...
  buyBtn.addEventListener('click', async () => {
    if (gameState.isSpinning || gameState.freeSpinsRemaining > 0) return;
    // Плашка покупки из макета — диалог с ✓/✗, поэтому указатель открывает
    // её, а деньги уходят только по ✓ (см. slot.js showBuyBonusDialog).
    const confirmed = await showBuyBonusDialog(BUY_BONUS_SPINS);
    if (!confirmed) return;
    const betAmount = gameState.betSteps[gameState.betIndex];
    // buy_id совпадает с продуктом bonus_buy в app/seed/big_catch.py.
