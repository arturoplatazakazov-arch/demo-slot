// Caesar's Fortune — bottom UI bar + spin orchestration, adapted from
// ../big-catch/app.js плюс расширяющийся вайлд родителя (wild_events из
// ответа /spin прокидываются в landReels). Session bootstraps on load; every
// Spin/Buy click is a real request against app/seed/caesars_fortune.py's
// config.
const GAME_ID = 'caesars-fortune';
...
// cost_multiplier продукта bonus_buy в app/seed/caesars_fortune.py. Число
// нужно только чтобы нарисовать цену под кнопкой и в диалоге; списывает
// сервер по своей конфигурации, что бы тут ни было показано.
...
// Цена под кнопкой покупки: в арте её нет вовсе, а равна она ставке x100 и
// едет вместе со ставкой.
...
async function runSpin(request) {
  ...
  try {
    ...
    const anticipationColumns = gameState.freeSpinsTrigger
      ...
      : [];
    await landReels(
      data.grid, anticipationColumns, data.wild_events || [],
      data.line_wins || [], data.count_wins || [],
    );
...
function setupBuyBonusButton() {
  ...
  buyBtn.addEventListener('click', async () => {
    ...
    if (!confirmed) return;
    const betAmount = gameState.betSteps[gameState.betIndex];
    // buy_id совпадает с продуктом bonus_buy в app/seed/caesars_fortune.py.
