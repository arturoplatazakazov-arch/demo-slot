// Uniqorn Scandal — adapted from ../mr-president-unicorn/slot.js, which runs
// the same Amy's Fruit Farm mechanic and the same PNG-instead-of-Spine
// presentation: reel motion, win-cell sequencing, scatter anticipation,
// free-spins mode, inline win amount, expanding wild, buy bonus.
//
// As in that game there are no skeletons, atlases or clips anywhere — the art
// is AI-generated flat PNGs and everything a skeleton used to animate is a
// DOM/CSS counterpart here:
//
...
const ASSET_ROOT = 'img/uniqorn-scandal';

const SYMBOL_CODES = ['wild', 'scatter', 'wife', 'heart', 'beer', 'a', 'k', 'q', 'j'];
...
//
//   plate   art                                                    used by
//   base    gold-and-pink heart cartouche, ribbons, light bulbs    big win, free spins, buy
//   mega    + gems, crown, coin shower, fireworks                  mega win, total win
//   epic    jewelled medallion, crowned unicorn crest, columns,    epic win
...
function applyStaticContentOffset(img, code) {
  const override = window.SlotCalibration && window.SlotCalibration.get('uniqorn-scandal', code);
...
// uniqorn-scandal.css body.layout-fit and ReelMath.fitScale.
// 480 (а не 360, как у mr-president): скуф с коробкой шире президента, при
// 360 рамка вырастает и он уезжает за левый край — та же правка, что
// понадобилась Uniqorn Back to Fabulous.
