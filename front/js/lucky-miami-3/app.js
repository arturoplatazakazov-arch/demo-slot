// Lucky Miami 3 — нижний бар и оркестрация спина. Адаптировано с
// ../uniqorn-shaolin-struggles/app.js минус всё, чего в этой игре нет: покупки
// бонуса, Hold & Win и любого второго режима экрана. Остаётся один путь —
// нажали спин, дождались ответа, уронили барабаны, показали выигрыш.

const GAME_ID = 'lucky-miami-3';
...
// пороги подобраны под здешнюю выплатную лестницу (вишня даёт 1.8x, семёрка —
// 1000x), так что bigWin начинается примерно с арбуза.
...
async function runSpin(request, betAmount) {
  ...
  try {
    ...
    // Подсказка под барабанами живёт ровно между спинами (референс держит её
    // там постоянно) — startReelLoop её прячет, вернуть её больше некому.
    setHintVisible(true);
  }
}
...
function renderInfoPopupContent() {
  ...
  for (const symbol of gameState.symbols) {
    ...
    row.innerHTML = `
      <img src="img/lucky-miami-3/symbols/${symbol.code}.png" alt="${symbol.name}">
