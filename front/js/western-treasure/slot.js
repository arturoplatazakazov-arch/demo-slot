// Western Treasure — adapted from ../gangsta-city/slot.js, which runs the same
...
// plus the Wheel of Fortune block lifted from ../dirty-money-mafia/slot.js —
// this game carries Empire of Crime's FULL mechanic set: line pay over 20
// lines, scatter free spins, expanding wild, bonus buy and the wheel.
//
// There are no skeletons, atlases or clips anywhere — the art is AI-generated
// flat PNGs (canyon-sunset art direction, see front/img/western-treasure/) and
// everything a skeleton used to animate is a DOM/CSS counterpart:
//
...
//   the wheel                    -> a CSS-rotated drum + DOM segment labels


const ASSET_ROOT = 'img/western-treasure';

// 11 symbols: 4 low (western props — horseshoe / revolver / whiskey /
// dynamite; deliberately NOT card royals, those read as the reference game's
// own set), 4 high (animal portraits on carved stone tiles: bison / mustang /
// eagle / wolf), the wild, the scatter and the wheel trigger.
//
// `wild` is the masked gunslinger's portrait plaque that lands on the grid;
// when it expands, the SAME character is drawn full-body, reel-tall, as
// wild_big.png (see revealExpandedWild) — that is the "full reel wild" of the
// spec, and the small/big pair is one character rather than two motifs.
const SYMBOL_CODES = [
  'wild', 'scatter', 'wheel',
  'bison', 'mustang', 'eagle', 'wolf',
...
const TRIGGER_CODES = new Set(['scatter', 'wheel', 'wild']);
...
//
//   idle    presenting the reels, open chest of gold     default
//   tense   braced, fists clenched, leaning in           while a scatter anticipation is running
//   cheer   arms up, hat off, coins raining              while a win is being paid out
const HERO_SRC = {
...
function setHeroMood(mood) {
  ...
  // The bison owns the BASE screen now (the two characters swapped screens),
  // and it has one drawing, not three. It still gets the same mood classes so
  // the base game reacts — the stylesheet gives it a CSS-only tense/cheer
  // instead of an art swap.
  const bison = document.querySelector('.bonus-hero');
  if (bison) {
    bison.classList.remove('is-idle', 'is-tense', 'is-cheer');
    bison.classList.add(`is-${mood}`);
  }
}

// Codes eligible as random spin-loop filler — excludes the three trigger
// symbols: scatter and wheel shouldn't blur past as filler, and wild expands,
// so it must not flicker through the blur either.
const FILLER_CODES = SYMBOL_CODES.filter((c) => !TRIGGER_CODES.has(c));

// wild, scatter and wheel idle permanently on the grid (a slow breathe/glow);
// every other symbol sits still until it wins. Purely a CSS class — see
// .reel__cell.is-special in the stylesheet.
const SPECIAL_SYMBOLS = new Set(['scatter', 'wild', 'wheel']);
...
const SYMBOL_LAYOUT = [
  ['scatter', 'bison', 'whiskey', 'horseshoe', 'dynamite'],
  ['horseshoe', 'revolver', 'wild', 'eagle', 'wheel'],
  ['whiskey', 'wolf', 'revolver', 'dynamite', 'mustang'],
];
...
//
//   plate   art                                                    used by
//   base    wood panel in a gold frame, rope, sheriff star         big win, free spins, buy
//   mega    + buffalo horns, gold bars, revolvers, coin shower     mega win, total win
//   epic    jewelled star medallion, crossed revolvers, bison      epic win
//           skull, velvet drapery, full firework burst
//
...
function applyStaticContentOffset(img, code) {
  const override = window.SlotCalibration && window.SlotCalibration.get('western-treasure', code);
...
// --- Wheel of Fortune -------------------------------------------------------
//
// Ported from ../dirty-money-mafia/slot.js (same feature, same server
// contract). Opened when the server reports 3+ `wheel` symbols (the spin
// response's `wheel_of_fortune`). The prize is ALREADY decided server-side —
// this only labels the drum from `segments`, waits for the player to hit SPIN,
// and eases the drum onto `segment_index`.

// Geometry read off the drum art (img/wof_drum.png), which is a REVOLVER
// CYLINDER (product) rather than a wagon wheel: 8 round chambers, the first
// one centred at 12 o'clock, so chamber i sits at i*45 degrees clockwise —
// hence OFFSET 0, unlike the wagon wheel whose spokes were on the boundaries
// and whose wedge centres were half a step over. The pointer stands at
// 3 o'clock (the frame's bison skull owns 12 o'clock — see .wof__pointer),
// which is why landing on chamber i means turning the drum by 90 - i*45.
// All of it is dictated by the artwork, and the chamber COUNT is the one
// number the seed's `segments` list has to agree with
// (app/seed/western_treasure.py).
const WOF_SEGMENT_COUNT = 8;
const WOF_SEGMENT_STEP = 360 / WOF_SEGMENT_COUNT;
const WOF_SEGMENT_OFFSET = 0;
...
// The wheel symbols pay nothing, so the server never lists them in
// `winning_cells` and playWinCells skips them — pulse them here so the trigger
// reads before the wheel takes over the screen.
async function celebrateWheelTrigger() {
  const triggers = cellInfos.filter((info) => info && info.symbol === 'wheel');
  if (triggers.length === 0) return;
  const winSet = new Set(triggers);
  for (const info of cellInfos) {
    if (info) setCellDimmed(info, !winSet.has(info));
  }
  await Promise.all(triggers.map((info) => playWinAnimationOnce(info)));
  for (const info of cellInfos) {
...
function renderWheelLabels(segments) {
  ...
  segments.slice(0, WOF_SEGMENT_COUNT).forEach((segment, i) => {
    ...
    spoke.style.setProperty('--a', `${WOF_SEGMENT_OFFSET + i * WOF_SEGMENT_STEP}deg`);
...
function playWheelOfFortune(wheel) {
  ...
  return new Promise((resolve) => {
    const start = () => {
      ...
      const target =
        WOF_SPIN_TURNS * 360 + WOF_POINTER_ANGLE
        - (WOF_SEGMENT_OFFSET + wheel.segment_index * WOF_SEGMENT_STEP);
...
// Dev-panel preview: a fake server payload so the wheel can be driven without
// a real 3-of-a-kind trigger.
function previewWheel() {
  const segments = [
    { type: 'multiplier', value: 2 }, { type: 'multiplier', value: 5 },
    { type: 'multiplier', value: 3 }, { type: 'multiplier', value: 10 },
    { type: 'multiplier', value: 2 }, { type: 'multiplier', value: 8 },
    { type: 'free_spins' }, { type: 'multiplier', value: 25 },
  ];
  const index = Math.floor(Math.random() * segments.length);
  return playWheelOfFortune({
    segments,
    segment_index: index,
    prize_type: segments[index].type,
    win_amount: (segments[index].value || 0) * 10000,
  });
}
...
function fitPopupText(node) {
  ...
  if (!body) return;
  const lines = [...body.querySelectorAll('.game-popup__title, .game-popup__amount, .game-popup__sub')];
...
function setupDevPanel() {
  ...
  const wheelBtn = document.getElementById('devWheel');
  if (wheelBtn) wheelBtn.addEventListener('click', () => previewWheel());
}

// Design authored at bg_base.jpg's native 1536-wide canvas; freeze the scale
// at 1280px and crop below that, same behavior as East Discovery.
const DESKTOP_DESIGN_MAX_WIDTH = 1536;
const DESKTOP_DESIGN_MIN_WIDTH = 1280;

// Experimental "fit everything, no scroll" layout — see
// gangsta-city.css body.layout-fit and ReelMath.fitScale.
// 480 (а не 360, как у mr-president): герой в полный рост занимает ту же
// полосу слева, что скуф с коробкой в Uniqorn Scandal — при 360 рамка
// вырастает и он уезжает за левый край.
// The hero stands in the lane to the left of the frame. He is twice as tall as
// he was, but he also sits BEHIND the frame (z-index 3 vs 6), so the lane does
// not have to hold all of him — 430 keeps his head and gun hand clear while
// leaving the reels the width they need. At 560 the width became the binding
// constraint of the fit and ate the whole 20% the reels were meant to gain.
const FIT_RESERVE_WIDTH_PX = 430;
...
// too: its height at that width, plus the gap. It was 250 while the reel block
// was 20% smaller — giving those 60px back is part of how the reels grew.
...
const FIT_RESERVE_HEIGHT_PX = 170;
...
function preloadAssets() {
  ...
  // The bonus hostess only appears once the bonus starts, but she swaps in at
  // the exact moment the blackout lifts — a late fetch would show an empty
  // rooftop.
  tasks.push(track(preloadImage(`${ASSET_ROOT}/img/bonus_hero.png`)));
