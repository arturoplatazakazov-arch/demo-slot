// Western Treasure — central sound manager, copied from
// ../wild-western-story/sound.js and pointed at that game's delivered pack.
// Unlike Gangsta City (which borrowed the same files but muted the music
// because a saloon score under a Miami-sunset skin was wrong), this theme IS
// the western one, so the background tracks play as delivered.
//
// Assets split two ways (see front/sound/README.md): repeating SFX shared by
// every game live in sound/common/; win tiers, background music and the
// wild-scale SFX come from the western pack. Each background level plays TWO
// looped layers at once — the music track plus an environment/atmosphere bed
// under it (see MUSIC_FILES).
...
const SOUND_FILES = {
  ...
  // NOTE: no `wildWin` entry. The western pack never shipped the longer bed
  // that goes under a wild win, and the sibling games keep the path anyway —
  // which costs a 404 on every page load of the public build. playSfx() no-ops
  // on an unknown key, so leaving it out is the same behaviour without the
  // failed request. Add it back the day the file lands.
  // shared / repeating (already present in sound/common/)
  // The wheel's wind-up borrows the reel anticipation cue: it is the same
  // "something is about to be decided" rise, and no wheel SFX was delivered.
  wheelSpin: COMMON + 'REELS_anticipation.mp3',
