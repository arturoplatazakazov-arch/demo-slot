// Western Treasure — bottom UI bar + spin orchestration, adapted from
// ../gangsta-city/app.js plus the wheel-of-fortune hook from
// ../dirty-money-mafia/app.js. Session bootstraps on load; every Spin/Buy
// click is a real request against app/seed/western_treasure.py's config.
const GAME_ID = 'western-treasure';
...
function renderInfoPopupContent() {
  ...
  for (const symbol of gameState.symbols) {
    ...
    // The wheel trigger has an empty paytable on purpose (it pays nothing on
    // its own — three of them open the wheel and the wheel is the prize), so
    // spell that out instead of leaving the row's pays column blank.
    const payParts =
      ['3', '4', '5']
        .filter((count) => symbol.paytable[count] !== undefined)
        .map((count) => `<span>×${count}: <b>${formatNumber(symbol.paytable[count])}</b></span>`)
        .join('') || '<span><b>×3 — BONUS WHEEL</b></span>';
    row.innerHTML = `
