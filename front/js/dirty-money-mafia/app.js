// Dirty Money / MAFIA — bottom UI bar + spin orchestration, adapted from
// ../wild-western-story/app.js (same mechanic set: line pay + scatter free
// spins + expanding wild + bonus buy). The session bootstraps on load and every
// Spin/Buy click is a real request against app/seed/dirty_money_mafia.py's
// config.

const GAME_ID = 'dirty-money-mafia';
...
// The number on Free_Spins_counter.png — the plate itself is static art.
...
function setupBuyBonusButton() {
  ...
  buyBtn.addEventListener('click', () => {
    ...
    const betAmount = gameState.betSteps[gameState.betIndex];
    // buy_id matches the bonus_buy product in app/seed/dirty_money_mafia.py.
...
// session-start); thumbnails reuse each symbol's own resting tile.
function renderInfoPopupContent() {
  ...
  for (const symbol of gameState.symbols) {
    ...
    const payParts = ['3', '4', '5']
      ...
      .join('');
    row.innerHTML = `
      ${folder ? `<img src="img/dirty-money-mafia/${folder}/${staticFileFor(symbol.code)}" alt="${symbol.name}">` : ''}
